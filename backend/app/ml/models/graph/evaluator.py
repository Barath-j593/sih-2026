"""
SETU — Stage 2H: Graph Model Evaluator.

Evaluates graph entity relationship anomaly detection against quarantined project labels:
...
class GraphEvaluator:
    """Evaluates graph relationship anomaly scores against quarantined labels."""
    ...
    def evaluate(
        ...
        """Run comprehensive post-hoc evaluation on graph relationship scores."""
        merged = scores_df.merge(labels_df, on="project_id")
        score_col = "graph_anomaly_score"
        n_total = len(merged)
        ...
        dist_stats = {
            ...
            "flagged_top_5pct": int((merged["is_graph_anomaly"] == 1).sum()),
        ...
        if baseline1_scores is not None and baseline2_scores is not None:
            for b_name, b_s in [("baseline1", baseline1_scores), ("baseline2", baseline2_scores), ("graph_model", merged[score_col])]:
        ...
        for scenario in hn_sub["scenario_type"].unique():
            ...
            flagged = int((sc_data["is_graph_anomaly"] == 1).sum())
            total = len(sc_data)
        ...
        for scenario, sc_data in merged.groupby("scenario_type"):
            ...
            flagged = int((sc_data["is_graph_anomaly"] == 1).sum())
            top_10 = int((sc_data["graph_anomaly_percentile"] >= 90.0).sum())
    ...
    def export_reports(self, report_data: Dict[str, Any]) -> Tuple[Path, Path]:
        ...
        json_path = self.config.reports_dir / "graph_model_report.json"
        md_path = self.config.reports_dir / "GRAPH_MODEL_REPORT.md"
        ...
        md_content = f"""# SETU Stage 2H — Graph & Entity Relationship Model Evaluation Report
...
| :--- | :---: | :---: | :---: | :---: |
| **Baseline 1: Tripartite Monopoly Rule** | {base_comp.get('baseline1', {}).get('top_1pct_fraud_rate', 'N/A')}% | {base_comp.get('baseline1', {}).get('top_5pct_fraud_rate', 'N/A')}% | {base_comp.get('baseline1', {}).get('top_10pct_fraud_rate', 'N/A')}% | {base_comp.get('baseline1', {}).get('top_10pct_enrichment', 'N/A')}x |
...
| **SETU Stage 2H Graph Isolation Forest** | **{base_comp.get('graph_model', {}).get('top_1pct_fraud_rate', 'N/A')}%** | **{base_comp.get('graph_model', {}).get('top_5pct_fraud_rate', 'N/A')}%** | **{base_comp.get('graph_model', {}).get('top_10pct_fraud_rate', 'N/A')}%** | **{base_comp.get('graph_model', {}).get('top_10pct_enrichment', 'N/A')}x** |
