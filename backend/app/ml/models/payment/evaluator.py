"""
SETU — Stage 2F: Payment Model Evaluator.

Evaluates payment anomaly detection against quarantined project labels:
...
class PaymentEvaluator:
    """Evaluates payment anomaly scores against quarantined labels."""
    ...
    def evaluate(
        ...
        """Run comprehensive post-hoc evaluation on payment scores."""
        merged = scores_df.merge(labels_df, on="project_id")
        score_col = "payment_anomaly_score"
        n_total = len(merged)
        ...
        dist_stats = {
            ...
            "flagged_top_5pct": int((merged["is_payment_anomaly"] == 1).sum()),
        ...
        if baseline1_scores is not None and baseline2_scores is not None:
            for b_name, b_s in [("baseline1", baseline1_scores), ("baseline2", baseline2_scores), ("payment_model", merged[score_col])]:
        ...
        for scenario in hn_sub["scenario_type"].unique():
            ...
            flagged = int((sc_data["is_payment_anomaly"] == 1).sum())
            total = len(sc_data)
            ...
            hard_negatives[scenario] = {
                ...
                "status": "Protected" if rate < 0.20 else "At Risk",
            }
        ...
        for scenario, sc_data in merged.groupby("scenario_type"):
            ...
            flagged = int((sc_data["is_payment_anomaly"] == 1).sum())
            top_10 = int((sc_data["payment_anomaly_percentile"] >= 90.0).sum())
    ...
    def export_reports(self, report_data: Dict[str, Any]) -> Tuple[Path, Path]:
        ...
        json_path = self.config.reports_dir / "payment_model_report.json"
        md_path = self.config.reports_dir / "PAYMENT_MODEL_REPORT.md"
        ...
        md_content = f"""# SETU Stage 2F — Granular Payment Structuring Model Evaluation Report
...
| :--- | :---: | :---: | :---: | :---: |
| **Baseline 1: Unverified & Round Rule** | {base_comp.get('baseline1', {}).get('top_1pct_fraud_rate', 'N/A')}% | {base_comp.get('baseline1', {}).get('top_5pct_fraud_rate', 'N/A')}% | {base_comp.get('baseline1', {}).get('top_10pct_fraud_rate', 'N/A')}% | {base_comp.get('baseline1', {}).get('top_10pct_enrichment', 'N/A')}x |
...
| **SETU Stage 2F Payment Isolation Forest** | **{base_comp.get('payment_model', {}).get('top_1pct_fraud_rate', 'N/A')}%** | **{base_comp.get('payment_model', {}).get('top_5pct_fraud_rate', 'N/A')}%** | **{base_comp.get('payment_model', {}).get('top_10pct_fraud_rate', 'N/A')}%** | **{base_comp.get('payment_model', {}).get('top_10pct_enrichment', 'N/A')}x** |
