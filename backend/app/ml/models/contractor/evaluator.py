"""Contractor Model Evaluator.

Evaluates unsupervised contractor anomaly detection performance against quarantined labels:
- Anomaly score distributions
- Fraud enrichment factors (top 1%, 5%, 10% vs baseline)
- Hard negative discrimination (is_hard_negative)
- Scenario detection coverage (SUSPICIOUS_CONTRACTOR_MONOPOLY)
...
class ContractorModelEvaluator:
    """Evaluator for Contractor & Agency Monopolization Anomaly Models."""
    ...
    def evaluate(
        ...
        scores = df["contractor_anomaly_score"].values
        distribution = {
            "total_projects": total_projects,
            "flagged_anomaly_count": int(df["contractor_anomaly_flag"].sum()),
            "flagged_anomaly_pct": float(df["contractor_anomaly_flag"].mean() * 100.0),
        ...
        def get_tier_stats(pct_cutoff: float):
            ...
            top_df = df.sort_values(by="contractor_anomaly_score", ascending=False).head(count)
        ...
        hard_negatives = {
            ...
            "flagged_hard_negative_count": int(df[df["contractor_anomaly_flag"]]["is_hard_negative"].sum()),
            "flagged_hard_negative_rate": float(df[df["contractor_anomaly_flag"]]["is_hard_negative"].mean()),
            "remote_single_bid_total": int(remote_mask.sum()),
            "remote_single_bid_flagged": int((remote_mask & df["contractor_anomaly_flag"]).sum()),
            "remote_single_bid_mean_score": float(df[remote_mask]["contractor_anomaly_score"].mean()) if remote_mask.sum() > 0 else 0.0,
            "high_value_legitimate_total": int(high_val_mask.sum()),
            "high_value_legitimate_flagged": int((high_val_mask & df["contractor_anomaly_flag"]).sum()),
            "high_value_legitimate_mean_score": float(df[high_val_mask]["contractor_anomaly_score"].mean()) if high_val_mask.sum() > 0 else 0.0,
            "weather_delay_total": int(weather_mask.sum()),
            "weather_delay_flagged": int((weather_mask & df["contractor_anomaly_flag"]).sum()),
            "weather_delay_mean_score": float(df[weather_mask]["contractor_anomaly_score"].mean()) if weather_mask.sum() > 0 else 0.0,
        }

        scenarios = {}
        for sc_name, sc_df in df.groupby("scenario_type"):
            sc_count = len(sc_df)
            flagged = int(sc_df["contractor_anomaly_flag"].sum())
            top10_count = int((sc_df["contractor_anomaly_percentile"] >= 90.0).sum())
            scenarios[str(sc_name)] = {
                ...
                "mean_anomaly_score": float(sc_df["contractor_anomaly_score"].mean()),
                "median_anomaly_score": float(sc_df["contractor_anomaly_score"].median()),
        ...
        if baseline1_scores is not None:
            ...
            baseline_comparison["baseline_1_agency_concentration"] = {
        ...
        if baseline2_scores is not None:
            ...
            baseline_comparison["baseline_2_multi_attribute_contractor_heuristic"] = {
                "top_1_pct_fraud_rate": float(top50["is_fraud"].mean()),
                "top_5_pct_fraud_rate": float(top250["is_fraud"].mean()),
                "top_10_pct_fraud_rate": float(top500["is_fraud"].mean()),
                "top_10_enrichment": float(top500["is_fraud"].mean() / base_fraud_rate) if base_fraud_rate > 0 else 1.0,
        ...
        top50_m = df.sort_values(by="contractor_anomaly_score", ascending=False).head(50)
        top250_m = df.sort_values(by="contractor_anomaly_score", ascending=False).head(250)
        top500_m = df.sort_values(by="contractor_anomaly_score", ascending=False).head(500)
        baseline_comparison["setu_stage_2e_contractor_isolation_forest"] = {
