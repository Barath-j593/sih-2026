"""Test Suite for SETU Stage 2E Contractor & Agency Monopolization Anomaly Model.
...
baseline benchmarks, reason trace explainability, and contractor collusion coverage.
"""
...
from app.ml.models.contractor.config import ContractorModelConfig
from app.ml.models.contractor.preprocessor import ContractorPreprocessor
from app.ml.models.contractor.contractor_model import ContractorIsolationForestModel
from app.ml.models.contractor.baseline_detectors import AgencyConcentrationBaseline, MultiAttributeContractorHeuristicBaseline
from app.ml.models.contractor.evaluator import ContractorModelEvaluator


@pytest.fixture(scope="module")
def config():
    return ContractorModelConfig()
...
def test_zero_target_and_identifier_leakage(config, master_df):
    ...
    preprocessor = ContractorPreprocessor(config)
...
def test_temporal_safety_early_warning(config, master_df):
    ...
    config_ew = ContractorModelConfig(mode="early_warning")
    preprocessor = ContractorPreprocessor(config_ew)
    preprocessor.fit(master_df)
...
def test_preprocessor_determinism(config, master_df):
    ...
    p1 = ContractorPreprocessor(config)
    ids1, df1, X1 = p1.fit_transform(master_df)

    p2 = ContractorPreprocessor(config)
...
def test_model_scoring_determinism(config, master_df):
    ...
    p = ContractorPreprocessor(config)
    _, _, X = p.fit_transform(master_df)

    m1 = ContractorIsolationForestModel(config)
    m1.fit(X, p.final_feature_names_)
    scores1 = m1.score(X)

    m2 = ContractorIsolationForestModel(config)
...
def test_score_range_and_validity(output_scores_df):
    ...
    scores = output_scores_df["contractor_anomaly_score"].values
    percentiles = output_scores_df["contractor_anomaly_percentile"].values
    flags = output_scores_df["contractor_anomaly_flag"].values
...
def test_model_artifacts_exist_and_loadable(config):
    ...
    model_path = os.path.join(artifacts_dir, "contractor_isolation_forest.joblib")
    preprocessor_path = os.path.join(artifacts_dir, "contractor_preprocessor.joblib")
    features_path = os.path.join(artifacts_dir, "contractor_features.json")
    config_path = os.path.join(artifacts_dir, "contractor_model_config.json")
    meta_path = os.path.join(artifacts_dir, "contractor_model_metadata.json")
    ...
    loaded_model = ContractorIsolationForestModel.load(model_path)
    loaded_prep = ContractorPreprocessor.load(preprocessor_path)
...
def test_baseline_comparison_validity(config, master_df):
    ...
    b1 = AgencyConcentrationBaseline(config)
    b1.fit(df_features)
    s1, p1, f1 = b1.score(df_features)
    assert len(s1) == 5000
    assert 0.0 <= s1.min() and s1.max() <= 100.0

    b2 = MultiAttributeContractorHeuristicBaseline(config)
    b2.fit(df_features)
...
def test_contractor_collusion_and_strain_coverage(output_scores_df):
    """Verify that projects with heavy capacity strain or corporate collusion are detected."""
    high_strain = output_scores_df[output_scores_df["capacity_strain_index"] > 1.0]
    if len(high_strain) > 0:
        assert high_strain["contractor_anomaly_score"].mean() > 40.0, "High capacity strain should receive elevated risk scores"

    high_collusion = output_scores_df[output_scores_df["corporate_collusion_index"] > 0.2]
    if len(high_collusion) > 0:
        assert high_collusion["contractor_anomaly_score"].mean() > 30.0, "Corporate collusion should receive elevated risk scores"
